...
namespace vt_darcy
{
    ...
    template <int dim>
    void
    st_to_subdom_distribute (BlockVector<double> &vector_st, std::vector<std::vector <std::vector<unsigned int>>> &interface_dofs_st,
        ...
        for (unsigned int side = 0; side < GeometryInfo<dim>::faces_per_cell; ++side)
              if (neighbors[side] >= 0)
            	  for(int i=0; i<interface_dofs_subd[side].size(); i++)
            		  vector_subdom[ interface_dofs_subd[side][i]] = vector_st[ interface_dofs_st[time_level][side][i]];
    ...
    template <int dim>
    void
    subdom_to_st_distribute (BlockVector<double> &vector_st, std::vector<std::vector <std::vector<unsigned int>>> &interface_dofs_st,
    						 BlockVector<double> &vector_subdom, std::vector <std::vector<unsigned int>> &interface_dofs_subd,
							 unsigned int &time_level,  const std::vector<int> &neighbors)
    {
        for (unsigned int side = 0; side < GeometryInfo<dim>::faces_per_cell; ++side)
              if (neighbors[side] >= 0)
            	  for(int i=0; i<interface_dofs_subd[side].size(); i++)
            		  vector_st[ interface_dofs_st[time_level][side][i]] = vector_subdom[ interface_dofs_subd[side][i]];
