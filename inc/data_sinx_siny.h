...
//        	return c0*t_scale*exp(t*t_scale)*sin(p[0])*sin(2.0*p[1]) + exp(t*t_scale)*5.0*sin(p[0])*sin(2.0*p[1]);
        	return c0*1.0*exp(t)*sin(p[0])*sin(p[1]) + exp(t)*2.0*sin(p[0])*sin(p[1]);
        default:
...
//          return exp(t*t_scale)*(cos(y*3.141592653589793)*sin(x*3.141592653589793)+1.0E1);
        	return exp(t*1.0)*sin(p[0])*sin(p[1]);
        default:
...
//                values(2) = exp(t*t_scale)*(cos(y*3.141592653589793)*sin(x*3.141592653589793)+1.0E1);
            	  ...
            	  values(0) = -exp(t*1.0)*cos(p[0])*sin(p[1]) ;
            	  values(1) = -exp(t*1.0)*sin(p[0])*cos(p[1]) ;
            	  values(2) = exp(t*1.0)*sin(p[0])*sin(p[1]);
...
//        	 grads[1][1] = exp(t*t_scale)*4.0*sin(p[0])*sin(2.0*p[1]);\
       	 ...
       	 grads[0][0] = exp(t*1.0)*sin(p[0])*sin(p[1]);;
       	 grads[0][1] = -exp(t*1.0)*cos(p[0])*cos(p[1]);

       	 grads[1][0] = -exp(t*1.0)*cos(p[0])*cos(p[1]);
       	 grads[1][1] = exp(t*1.0)*sin(p[0])*sin(p[1]);
...
//              values(2) = exp(t*t_scale)*sin(p[0])*sin(2.0*p[1]);
              values(2) = exp(t)*sin(p[0])*sin(p[1]);
          break;
          case 3:
